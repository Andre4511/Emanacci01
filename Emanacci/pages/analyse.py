...
def render_analyse_page() -> None:
    ...
    if "zoom" not in st.session_state:
        st.session_state.zoom = st.session_state.last_zoom
    ...
    if trend == "Bullisch":
        score += 30
        score_details.append("Trend: gut (+30)")
    elif trend == "Neutral":
        score += 15
        score_details.append("Trend: neutral (+15)")
    else:
        score_details.append("Trend: schwach (+0)")
        improvements.append("Achte auf einen klareren Aufwärtstrend")

    # Signal
    if signal == "Positiv":
        score += 40
        score_details.append("Signal: gut (+40)")
    elif signal == "Unklar":
        score += 20
        score_details.append("Signal: unklar (+20)")
        improvements.append("Warte auf eine klare Bestätigung")
    else:
        score_details.append("Signal: negativ (+0)")
        improvements.append("Vermeide Einstieg bei negativem Signal")

    # Risiko
    if risk == "Niedrig":
        score += 30
        score_details.append("Risiko: gut (+30)")
    elif risk == "Mittel":
        score += 15
        score_details.append("Risiko: mittel (+15)")
        improvements.append("Optimiere dein Risiko (z. B. besserer Stop-Loss)")
    else:
        score_details.append("Risiko: hoch (+0)")
        improvements.append("Reduziere dein Risiko deutlich")
    ...
    st.markdown('<div class="em-section-title">Deine Einschätzung</div>', unsafe_allow_html=True)
    ...
    if trend == "Bullisch":
        ...
        check_items.append(("Trend unklar", "neutral"))
    ...
    # =====================================================
    # TRADE BEWERTUNG (JETZT RICHTIG)
    # =====================================================

    trade_feedback = ""
    trade_color = "neutral"

    if signal == "Positiv" and risk == "Niedrig":
        trade_feedback = "Setup wirkt solide – Trade möglich."
        trade_color = "good"

    elif signal == "Unklar":
        trade_feedback = "Signal noch nicht bestätigt – besser abwarten."
        trade_color = "neutral"

    elif risk == "Hoch":
        trade_feedback = "Risiko aktuell hoch – Einstieg eher ungünstig."
        trade_color = "bad"

    else:
        trade_feedback = "Setup gemischt – nur mit Vorsicht handeln."
        trade_color = "neutral"

    if trade_color == "good":
        st.success(trade_feedback)
    elif trade_color == "bad":
        st.error(trade_feedback)
    else:
        st.warning(trade_feedback)

    if trade_color == "bad":
        st.info("💡 Tipp: Überlege, ob du diesen Trade wirklich eingehen möchtest.")
    ...
    if st.session_state.confirm_trade:
        ...
        with c1:
            if st.button("✅ Ja, übernehmen", use_container_width=True):
