...
def render_analyse_page() -> None:
    ...
    if signal == "Positiv" and rr < 1.5:
        next_steps.append("Chance/Risiko noch nicht optimal")

    if trend == "Bärisch":
        next_steps.append("Nicht gegen den Trend handeln")

    risk_pct = abs(entry - sl_price) / entry if entry else 0

    if risk_pct > 0.03:
        next_steps.append("Risiko reduzieren oder Stop-Loss anpassen")

    if not next_steps:
        next_steps.append("Setup weiter beobachten")  
    ...
    with st.expander("📘 Was bedeuten die EMA-Linien?"):
        st.caption("Die EMA zeigt dir den durchschnittlichen Preis über einen Zeitraum.")
        st.caption("Kurze EMA (z. B. 20) reagiert schnell auf Änderungen.")
        st.caption("Lange EMA (z. B. 50 oder 200) zeigt den langfristigen Trend.")

        st.info("👉 Wenn die kurze EMA über der langen liegt, ist der Trend oft positiv.")

    with st.expander("📘 Was bedeutet der Trend hier?"):
        st.caption("Ein Trend zeigt dir die aktuelle Richtung des Marktes.")

        st.caption("• Steigend = Käufer dominieren")
        st.caption("• Fallend = Verkäufer dominieren")
        st.caption("• Seitwärts = Unsicherheit")

        st.info("👉 Anfänger sollten möglichst mit dem Trend handeln.")

    with st.expander("📘 Was bedeutet das Signal?"):
        st.caption("Das Signal fasst mehrere Faktoren zusammen.")

        st.caption("• Positiv = eher Einstieg möglich")
        st.caption("• Unklar = lieber abwarten")
        st.caption("• Negativ = eher kein Einstieg")
    ...
    with st.expander("Was im Chart passiert", expanded=False):
        if latest_price > ema20 > ema50:
            st.caption("- Trend wirkt stabil nach oben (über EMA).")
        elif latest_price < ema20 < ema50:
            st.caption("- Abwärtstrend intakt (unter EMA).")
        else:
            st.caption("- Trend nicht klar – EMA Linien gemischt.")

        if latest_rsi > 70:
            st.caption("- RSI hoch – Markt könnte überkauft sein.")
        elif latest_rsi < 30:
            st.caption("- RSI niedrig – mögliche Gegenbewegung.")
        else:
            st.caption("- RSI im normalen Bereich.")
    ...
    if signal == "Unklar":
        st.caption("Warte auf eine klarere Bewegung oder Bestätigung.")
    elif signal == "Positiv":
        st.caption("Du kannst den Markt beobachten oder vorsichtig einsteigen.")
    else:
        st.caption("Lieber abwarten oder Risiko reduzieren.")
