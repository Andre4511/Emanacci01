...
def render_journal_page() -> None:
    ...
    if top_mistakes:
        ...
        st.info("Noch nicht genug Daten für eine Auswertung.")
    ...
    for _, row in latest_trades.iterrows():
        with st.container(border=True):
            ...
            if str(row.get("main_mistake", "-")) not in ["-", "", "nan"]:
                st.caption(f"Wichtigster Fehler: {row['main_mistake']}")
    ...
    if row.get("main_mistake"):
        st.caption(f"❗ {row['main_mistake']}")

    if row.get("learning_step"):
