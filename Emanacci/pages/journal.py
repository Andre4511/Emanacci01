...
def render_journal_page() -> None:
    ...
    # =====================================================
    # FEHLER ANALYSE (SMART)
    # =====================================================

    st.subheader("🧠 Dein Trading-Verhalten")

    if not filtered_df.empty:

        # Fehler zählen
        mistake_counts = (
            filtered_df["main_mistake"]
            .dropna()
            .astype(str)
            .value_counts()
        )
        ...
        if not mistake_counts.empty:
            ...
            top_mistake = mistake_counts.idxmax()
            top_count = mistake_counts.max()

            st.error(f"❗ Häufigster Fehler: {top_mistake}")
            st.caption(f"{top_count}x passiert")

            # Einfache Diagnose
            if "früh" in top_mistake.lower():
                st.warning("👉 Du steigst oft zu früh ein – warte auf Bestätigung.")
            elif "trend" in top_mistake.lower():
                st.warning("👉 Du handelst öfter gegen den Trend.")
            elif "risiko" in top_mistake.lower():
                st.warning("👉 Dein Risiko ist oft zu hoch.")
            else:
                st.info("👉 Beobachte dein Verhalten weiter.")

        else:
            st.info("Noch keine Fehler erkannt.")
    ...
    for _, row in latest_trades.iterrows():
        with st.container(border=True):
            ...
            if row.get("main_mistake") and str(row["main_mistake"]) not in ["", "-", "nan"]:
                st.error(f"❗ {row['main_mistake']}")
