...
def render_paper_trading_page() -> None:
    ...
    with row1_right:
        ...
        if positions:
            ...
            p1.metric("Offene", len(positions))
            p2.metric("Bester", f"{best_trade['symbol']} ({best_trade['pnl']:.2f})")
            p3.metric("Schwächster", f"{worst_trade['symbol']} ({worst_trade['pnl']:.2f})")
        else:
    ...
    if not positions:
        ...
        for pos in positions:
            ...
            qty = float(pos["qty"])
            investment = float(pos.get("investment_amount", entry * qty))
            ...
            with st.container(border=True):
                ...
                # Header
                st.markdown(f"### {pos['symbol']} — {status}")
                ...
                with c2:
                    st.caption(f"Aktuell: {current_price:.2f}")
                ...
                # Betrag
                st.caption(f"Betrag: {investment:.2f} €")

                # Optional: Hinweis aus Analyse
                if pos.get("main_mistake") not in ["-", "", None]:
                    st.caption(f"❗ {pos['main_mistake']}")

                # Buttons
                b1, b2 = st.columns(2)

                with b1:
                    if st.button("Teilverkauf", key=f"partial_{pos['id']}", use_container_width=True):
                        st.info("Teilverkauf kommt später")

                with b2:
                    if st.button("Schließen", key=f"close_{pos['id']}", use_container_width=True):
                        exit_price = current_price
                        pnl_abs = (exit_price - entry) * qty
                        pnl_pct = ((exit_price - entry) / entry) * 100 if entry else 0

                        st.session_state.paper_cash += qty * exit_price
                        st.session_state.paper_positions = [
                            p for p in positions if p["id"] != pos["id"]
                        ]

                        _append_journal_entry(pos, exit_price, pnl_abs, pnl_pct)
    ...
    with row4_left:
        ...
        if st.session_state.equity_curve:
            st.line_chart(st.session_state.equity_curve, use_container_width=True)
